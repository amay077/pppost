...
- [x] 1.1 `backend/netlify/functions/youtube_oembed.js` を新規作成する
...
- [x] 1.2 `backend/test.http` に `youtube_oembed` のテスト項目を追記する

## 2. フロントエンド（MainContent.svelte）

- [x] 2.1 YouTube URL の検出・正規化関数を追加する
...
- [x] 2.2 oEmbed タイトル取得処理を追加する（`/youtube_oembed` を呼び出し、ローディング状態を管理）
- [x] 2.3 onMount の処理フローに YouTube 処理を組み込む
