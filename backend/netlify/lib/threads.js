...
const { getPrGhostState, updatePrGhostExecState } = require('./pr-ghost');
...
// label は失敗時のログで作成段階（text / image / carousel-item / carousel / ghost / video）を識別するために使う
...
// 動画コンテナが予算内に FINISHED にならない場合は、失敗とせず
// { ok: false, statusCode: 202, creationId } を返す（クライアントが threads_video_finalize で後続処理する）。
...
const doThreadsPostOnce = async ({ token, text, imageUrls, videoUrl, reply_to_id, quote_to_id, isGhost, deadline }) => {
  ...
  if (!ready) {
    // 動画コンテナは処理に時間がかかる（Meta 側で数十秒かかることがある）ため、
    // 予算内に完了しない場合は失敗とせず、後続の最終化（threads_video_finalize）で
    // 公開できるよう 202 と creation_id を返す（動画投稿のみ）
    if (videoUrl != null) {
      return { ok: false, statusCode: 202, creationId: creation_id };
    }
...
// それ以外の失敗は 1 回で失敗とする。動画の 202（creationId）はリトライ対象外としてそのまま返す。
// isGhost=true のときはテキストのみ（media_type=TEXT）で is_ghost_post を付与し、画像・動画は無視する。
// deadline は絶対時刻（ミリ秒）で、この投稿に使える実行時間の上限を表す。
