...
// drive/files/upload-from-url による非同期の drive 取り込みが完了するのを待ち、
// 対象ファイルを drive から探してノートを作成する。
...
const handler = async (event) => {
  ...
  try {
    ...
    if (!filesRes.ok) {
      console.error(`Failed to fetch drive files:`, filesRes.status, await filesRes.text());
